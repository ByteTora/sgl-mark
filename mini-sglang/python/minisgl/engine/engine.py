...
class Engine:
    def __init__(self, config: EngineConfig):
        ...
        from minisgl.watermark import WatermarkLogitsProcessor
    ...
    def forward_batch(self, batch: Batch, args: BatchSamplingArgs) -> ForwardOutput:
        ...
        # Apply watermarking to logits if enabled for any request in the batch
        processed_logits = self.watermark_processor.process_batch(
            batch, logits[: batch.size], args
        )
        
        # IMPORTANT: Watermark processor already applied temperature scaling
        # (scores / temperature). If we pass the same args to sampler, it will
        # divide by temperature AGAIN.
        # We need to neutralize the sampler's temperature if watermarking was applied.
        from copy import copy
        modified_args = args
        watermark_enabled_mask = [req.sampling_params.watermark_enabled for req in batch.reqs]
        
        if any(watermark_enabled_mask) and args.temperatures is not None:
            # For watermarked requests, we want the sampler to treat them as T=1.0
            # because the logits are already scaled.
            new_temps = args.temperatures.clone()
            # Find indices where watermark is enabled
            for i, enabled in enumerate(watermark_enabled_mask):
                if enabled:
                    new_temps[i] = 1.0
            modified_args = copy(args)
            modified_args.temperatures = new_temps

        next_tokens_gpu = self.sampler.sample(processed_logits, modified_args).to(torch.int32)
        
        # CRITICAL: Inform the watermark processor which tokens were actually sampled
        # so it can update its internal history for the next step.
        self.watermark_processor.update_last_token(batch, next_tokens_gpu)
